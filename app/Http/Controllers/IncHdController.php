    ...
    public function index()
    {
        ...
        $amount_qc_users = DB::connection('mysql_main_qc')
            ...
            ->select('qu.emp_name', 'ld.empqc',
                ...
                DB::raw('SUM(CASE
            WHEN lv.levelname = "Very easy" THEN 1
            ELSE 0
        END) AS level_very_easy'),
                DB::raw('SUM(CASE
            WHEN lv.levelname = "Easy" THEN 1
            ELSE 0
        END) AS level_easy'),
                DB::raw('SUM(CASE
            WHEN lv.levelname = "Middling" THEN 1
            ELSE 0
        END) AS level_middling'),
                DB::raw('SUM(CASE
            WHEN lv.levelname = "Hard" THEN 1
            ELSE 0
        END) AS level_hard'),
                DB::raw('SUM(CASE
            WHEN lv.levelname = "Very Hard" THEN 1
            ELSE 0
        END) AS level_very_hard'),
            )
